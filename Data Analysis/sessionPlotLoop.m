...
baselineFemales = ["alexis","harley","renata","sarah","shakira"];
% Set Thresholds
minimumFit = 0.4;
minAppRate = 0.25;

fractionOfSigmoid = zeros(1,length(baselineFemales));
appRateFracGTthr = zeros(1,length(baselineFemales));

for animal = 1:length(baselineFemales)
    liveTableQuery = sprintf("SELECT id, genotype, subjectid, health, referencetime, " + ...
        "gender, feeder FROM live_table WHERE subjectid = '%s' ORDER BY id;", baselineFemales(animal));
    ...
    for index = 1:length(allGoodness) % according to the health you want
        ...
        label = sprintf('%s: %s, %d trials',baselineFemales(animal),uniqueDates(startIndex+index-1), ...
            height(dataOnDate));
